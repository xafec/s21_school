...
/*
    Для работы обязательно использовать следующие расширения vscode:
    - clangd
    - Todo Tree
*/

/*
    TODO: Заменить для ball позиции X и Y ( ball_x , ball_y , b_vx , b_vy ), так как у нас они перепутаны
*/
...
int main() {
    ...
    while (!winner) {
        ...
        if (ball_y == 5 && (ball_x == P1_y || ball_x == P1_y + 1 || ball_x == P1_y - 1) && (b_vy == 1)) {
            // Если шар ударяется о границу а потом попадает на ракетку, то пусть продолжает движение, а не
            // меняет направление
...
void control_menu(int score_1, int score_2) {
    ...
    printf("Player 1: 'a' - up, 'z' - down\t\t\t   Player 2: 'k' - up, 'm' - down\n\n");
    printf("\t\t\t\t    Exit: 'q'\n");
}
